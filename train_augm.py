...
import graphs_augm.convnet as conv
...
DATASET_NAME='train_norm_data.npy'
...
# load train data
train_data = np.load(os.path.join(DATASET_DIR,DATASET_NAME))
print('Data loaded')
...
#     conv2_model.fit({'input':X},{'targets':Y}, n_epoch=100, validation_set=({'input':test_x},{'targets':test_y}), snapshot_step=5000, show_metric=True, run_id='conv2')
...
# convnet_model = tflearn.DNN(convnet_cnn, tensorboard_dir='log')
...
if os.path.exists(os.path.join('{}.meta'.format(CNN6))):
    ...
    # TRAIN the network run_id is how you will find in tensorflow the model
    conv6_model.fit({'input':X},{'targets':Y}, n_epoch=100, validation_set=({'input':test_x},{'targets':test_y}), snapshot_step=5000, show_metric=True, run_id='conv6')
